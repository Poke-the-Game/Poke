...
function Snake(player) {
    ...
    this.default_length = (player.game.game_type=='classic')?12:8;
...
Snake.prototype.transform_blocks = function() {
    if(this.state == "frozen" || this.state == "dead")
        return;
...
Powerup.prototype.on_touch = function(player) {
    var opponent = this.game.get_opponent(player);
    ...
    if(this.type == 'freeze_snake') {
        opponent.snake.freeze(5000);
    }

    if(this.type == 'flip_screen') {
        opponent.connection.emit('flip_screen', {duration: 15000});
    }

    if(this.type == 'beer') {
        opponent.connection.emit('beer', {duration: 12000});
    }
    ...
    if(this.type == 'mushroom') {
        opponent.connection.emit('mushroom_field', {duration: 12000});
